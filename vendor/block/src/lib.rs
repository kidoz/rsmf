...
This is a vendored fork of upstream `block` 0.1.6 (Steven Sheldon, MIT). The
only change is the private `Class` placeholder: upstream declared it as an
uninhabited `enum Class {}`, which made `static _NSConcreteStackBlock: Class`
trigger the `static of uninhabited type` future-incompatibility lint
(rust-lang/rust#74840). It is replaced here with an inhabited opaque type so the
static is legal. `Class` is private and only ever used as `*const Class`, so the
public API and ABI are unchanged.
*/
...
// Opaque Objective-C class placeholder. Upstream used an uninhabited
// `enum Class {}`; an inhabited zero-sized opaque type keeps the same `*const
// Class` usage while making `static _NSConcreteStackBlock: Class` legal.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
